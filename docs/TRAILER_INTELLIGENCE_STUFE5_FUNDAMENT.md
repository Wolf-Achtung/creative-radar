...
## 7. Warum die Prüfung einen Umweg nehmen musste

`alembic upgrade head` bricht auf einer **frischen, leeren** Datenbank ab:
...
**Und es ist auch nichts Neues.** Der Kommentar im CI-Workflow
(`.github/workflows/backend-tests.yml`, Schritt „Bootstrap schema") beschreibt genau
dieses Problem samt Ursache:

> the autogenerated baseline migration `cf842bbfaeb5` lacks `schema='creative_radar'`
> arguments AND the post-baseline migrations […] CREATE TYPEs that SQLAlchemy/SQLModel
> already creates implicitly from Enum columns — so a fresh `alembic upgrade head`
> collides on duplicate enums.

Deshalb bootstrappt CI über `SQLModel.metadata.create_all` und stempelt anschließend
`alembic stamp head`, statt die Kette laufen zu lassen. Der Workflow benennt auch den
Weg heraus: *„a separate sprint that backfills `schema='creative_radar'` into the
baseline migration and de-duplicates the enum-creation paths."*

Neu ist hier allein das genaue Symptom auf einer wirklich leeren Datenbank — nicht der
vorhergesagte Duplikat-Konflikt, sondern das Gegenstück dazu: die Kette legt den Typ
`market` nicht in `public` an, eine spätere Migration will ihn dort aber erweitern.
Dieselbe Wurzel, andere Seite.

Nicht dringend: Produktion und Staging sind längst migriert, die Kette wird dort nie
wieder von null gestartet. Es trifft nur, wer eine Datenbank neu aufsetzt. Der Umweg für
die Prüfung war deshalb: auf `b2e8f4a6c1d7` stempeln, dann `upgrade head` — damit läuft
genau die eine neue Migration und sonst nichts.

Bemerkenswert bleibt eine Folge davon: **CI führt die Migrationskette nie aus.** Eine
kaputte Migration fiele dort nicht auf. Genau deshalb ist die Prüfung aus Abschnitt 6.1
von Hand gelaufen.
