...
| ORM-Code referenziert `creative_radar.X` BEVOR Migration läuft | Reihenfolge in 4.1c/4.1d: ORM-Schema-Setting (4.1c) wird gepusht, Code geht live, dann Migration läuft (4.1d). Während des Zeitfensters zwischen Code-Deploy und Migration-Endpoint-Aufruf wäre das Backend kaputt — daher: 4.1c und 4.1d in **derselben Deploy-Welle** (4.1c-Push triggert Railway-Redeploy, sofort danach 4.1d-curl). **Update nach Crash-Hotfix:** der `create_db_and_tables`-on-startup-Hook ist für Postgres deaktiviert, sonst würde `metadata.create_all` (a) bei fehlendem Schema crashen, (b) mit Schema leere Tabellen anlegen, die die `ALTER TABLE SET SCHEMA`-Migration blockieren. Siehe `PHASE_4_DONE.md` Lesson 1. |
| FK-Strings unqualifiziert mit aktivem Schema | `_fk(target)`-Helper in `app/models/entities.py` qualifiziert FK-Targets nur für Postgres-Mode. SQLite-Tests lassen die unqualifizierte Form, weil `_resolve_table_schema()` dort `None` returnt. Regression-Schutz via `tests/test_orm_fk_resolution.py` (Postgres-Mode-Subprocess). Siehe `PHASE_4_DONE.md` Lesson 2 + 3. |
