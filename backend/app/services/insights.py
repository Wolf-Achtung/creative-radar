...
def _score(asset: Asset, post: Post | None) -> int:
    ...
    if asset.review_status == ReviewStatus.APPROVED or asset.include_in_report:
        review_bonus += 350
    confidence = int((asset.confidence_score or 0) * 100)
    return visible + review_bonus + confidence
...
def _asset_payload(session: Session, asset: Asset) -> dict[str, Any]:
    ...
    return {
        ...
        "title": title.title_original if title else "Discovery / kein Whitelist-Match",
...
def build_overview(session: Session, week_start: date | None = None, week_end: date | None = None) -> dict[str, Any]:
    ...
    for asset in assets:
        ...
        channel = session.get(Channel, post.channel_id) if post else None
        title = session.get(Title, asset.title_id) if asset.title_id else None
        key_title = title.title_original if title else "Discovery / kein Whitelist-Match"
    ...
    if missing_previews:
        recommendations.append(f"{missing_previews} Assets haben kein Preview-Bild. Apify-Bildfelder oder Screenshot-Service priorisieren.")
    ...
    if not any(item.get("visible_views") or item.get("visible_likes") or item.get("visible_comments") for item in cards):
        recommendations.append("Aktuell keine belastbaren sichtbaren Interaktionswerte vorhanden; Ranking basiert vorläufig auf Review-Status und Confidence.")
