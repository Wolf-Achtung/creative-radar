"""Tests für den Trockenlauf-Lesezugriff auf das Cutter-Wochenbriefing
(``GET /api/admin/cutter-weekly/latest`` — Commit E).
...
@pytest.fixture
def db():
    fd, path = tempfile.mkstemp(prefix="cr_admin_cutter_", suffix=".db")
    os.close(fd)
...
def _seed_row(db, iso_year: int, iso_week: int, *, model: str = "claude-opus-4-7"):
    with Session(db) as session:
        session.add(
...
def test_latest_returns_most_recent_week(client, db):
    ...
    resp = client.get("/api/admin/cutter-weekly/latest", headers=AUTH)
...
def test_exact_week_lookup_and_404(client, db):
    ...
    ok = client.get(
        "/api/admin/cutter-weekly/latest",
...
def test_empty_table_is_404(client):
    ...
    assert "Noch kein Cutter-Wochenbriefing" in resp.json()["detail"]


def test_endpoint_requires_bearer_auth(client, db):
    _seed_row(db, 2026, 22)
    resp = client.get("/api/admin/cutter-weekly/latest")
