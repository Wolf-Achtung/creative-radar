"""Cron-Integration des Cutter-Wochenbriefings (Commit D).

Verifiziert ``_run_cutter_weekly_after_forecasts`` plus Reihenfolge im
...
2. F0.7-Cap-Re-Check: hard-cap exceeded → Block skippt, Generator wird
   nicht gerufen (Briefs/Roundups/Einordnungen sind dann schon durch).
...
7. Reihenfolge: Forecast-Warmup VOR dem Cutter-Block (Signal-Read
   trifft nur Cache), Summary trägt den cutter_weekly-Block.
"""
...
from app.models.entities import CronRun, CutterWeeklyBriefing


@pytest.fixture
def db():
    fd, path = tempfile.mkstemp(prefix="cr_cron_cutter_", suffix=".db")
    os.close(fd)
...
def _patch_generate(monkeypatch, fn):
    ...
    monkeypatch.setattr(
        "app.services.cutter_weekly.generate_and_persist_cutter_weekly", fn
    )
...
def test_cutter_weekly_skipped_when_feature_flag_off(db, monkeypatch):
    monkeypatch.delenv("FEATURE_CUTTER_WEEKLY_ENABLED", raising=False)
    gen_mock = MagicMock()
    _patch_generate(monkeypatch, gen_mock)

    with Session(db) as session:
        result = cron_module._run_cutter_weekly_after_forecasts(session)
...
def test_cutter_weekly_skipped_when_anthropic_cap_exceeded(db, monkeypatch):
    monkeypatch.setenv("FEATURE_CUTTER_WEEKLY_ENABLED", "true")
    monkeypatch.setattr(
...
def test_cutter_weekly_runs_when_cap_exceeded_but_enforced_false(db, monkeypatch):
    monkeypatch.setenv("FEATURE_CUTTER_WEEKLY_ENABLED", "true")
    monkeypatch.setattr(
        cron_module, "compute_anthropic_monthly_spend",
...
def test_cutter_weekly_cache_hit_skips_llm_call(db, monkeypatch):
    monkeypatch.setenv("FEATURE_CUTTER_WEEKLY_ENABLED", "true")
    monkeypatch.setattr(
        cron_module, "compute_anthropic_monthly_spend",
    ...
    with Session(db) as session:
        result = cron_module._run_cutter_weekly_after_forecasts(
...
def test_cutter_weekly_force_overrides_cache_hit(db, monkeypatch):
    monkeypatch.setenv("FEATURE_CUTTER_WEEKLY_ENABLED", "true")
    monkeypatch.setattr(
        cron_module, "compute_anthropic_monthly_spend",
        lambda s: _make_budget_status(exceeded=False),
...
def test_cutter_weekly_success_summary(db, monkeypatch):
    monkeypatch.setenv("FEATURE_CUTTER_WEEKLY_ENABLED", "true")
    monkeypatch.setattr(
        cron_module, "compute_anthropic_monthly_spend",
        lambda s: _make_budget_status(exceeded=False),
    ...
    # Briefs/Roundups desselben Laufs.
...
def test_cutter_weekly_failure_is_isolated(db, monkeypatch):
    monkeypatch.setenv("FEATURE_CUTTER_WEEKLY_ENABLED", "true")
    monkeypatch.setattr(
        cron_module, "compute_anthropic_monthly_spend",
        lambda s: _make_budget_status(exceeded=False),
    ...
    def boom(session, **kwargs):
        raise RuntimeError("simulated cutter failure")
...
# ---------------------------------------------------------------------------
# Test 7 — Reihenfolge im Cron-Background: Warmup vor Cutter-Block
# ---------------------------------------------------------------------------


def test_cron_background_runs_warmup_before_cutter_weekly(db, monkeypatch):
    """Position ist Absicht: der Signal-Read des Briefings trifft nach dem
    Einordnungs-Warmup nur Cache. Verifiziert via call-order der Spies;
    Summary trägt den cutter_weekly-Block."""
    call_order: list[str] = []
    ...
    assert call_order == ["er_forecasts", "cutter_weekly"]

    with Session(db) as session:
        run = session.get(CronRun, run_id)
        assert run.status == "completed"
        assert "cutter_weekly" in run.summary_json
        assert run.summary_json["cutter_weekly"]["generated"] == 1
