...
def _evidence_post(url: str, key: str = "Film A") -> WeeklyEvidencePost:
...
def _released_ig(urls: list[str]) -> WeeklyPlatformEvidence:
    posts = [
...
def _idle(platform: str, status: str = "no_pattern") -> WeeklyPlatformEvidence:
    return WeeklyPlatformEvidence(
        platform=platform,
        status=status,
...
def _evidence(
    ...
    signals: Optional[list[WeeklyForecastSignal]] = None,
) -> WeeklyBriefingEvidence:
...
def test_validator_accepts_signal_note_with_signals():
    signals = [
        WeeklyForecastSignal(pair_key="disney", market="DE", direction="steigend", n_points=6)
    ]
